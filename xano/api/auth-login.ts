...
// Sign-in. Verifies the credential and mints a bearer token, returning the
// caller's role so the frontend can show who is signed in. The password is taken
// as `input.text()` (never `input.password`, which would double-hash) and the
// stored hash is pulled with an explicit `output` because the password column is
// internal.
export const loginQuery = query({
  ...
  responseShape: null as unknown as {
    ...
    email: string;
    name: string;
    role: "pricing-admin" | "app-service";
  },
});
